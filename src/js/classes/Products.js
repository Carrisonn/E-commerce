...
import { divCartUserProducts } from '../cart/constants.js';
...
export class Products {
  ...
  renderSingleProduct(product) {
    ...
    const divProduct = document.createElement('div');
    divProduct.classList.add('single-product-card');
    divProduct.innerHTML = `
    ...
    `;
    divProductContainer.appendChild(divProduct);
  }
  ...
  renderCartProducts(products) {
    products.forEach(product => {
      ...
      divProduct.classList.add('product-card');
      divProduct.innerHTML = `
        <img src="${image}" class="product-image" title="${title}" alt="${title}">
        <h3 class="product-title">${title}</h3>
        <div class="div-product-cart-price-delete">
          <p class="price">Price: <span class="price-span">${price}</span></p>
          <button class="btn-delete-product" data-id="${id}"> Delete</button>
        </div>
      `;
      divCartUserProducts.appendChild(divProduct);
    });
  }
